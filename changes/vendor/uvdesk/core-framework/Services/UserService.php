...
class UserService
{
    ...
    public function getLocalizedFormattedTime(\DateTime $timestamp, $user = null, $format = 'F j, Y h:i A')
    {
        ...
        if (!empty($user) && $user != 'anon.' && $user->getTimezone() != null) {
            ...
            $format = $user->getTimeFormat() ?: 'F j, Y h:i A';
            ...
            $format = $activeUserTimeZone->getTimeFormat() ?: 'F j, Y h:i A';
        }
    ...
    /**
     * Estimate ticket handling time based on historical data
     * Returns estimated time in minutes
     */
    public function estimateTicketTime()
    {
        $today = new \DateTime('today');
        $todayEnd = new \DateTime('today 23:59:59');

        // 1. Try to get tickets from today
        $todayHandlingTimes = $this->getTicketHandlingTimes($today, $todayEnd);
        if (!empty($todayHandlingTimes)) {
            return $this->getMedianTime($todayHandlingTimes);
        }

        // 2. Look back through recent days (7-14 days)
        for ($daysBack = 1; $daysBack <= 14; $daysBack++) {
            $dayStart = new \DateTime("-{$daysBack} days");
            $dayStart->setTime(0, 0, 0);
            $dayEnd = clone $dayStart;
            $dayEnd->setTime(23, 59, 59);

            $dayHandlingTimes = $this->getTicketHandlingTimes($dayStart, $dayEnd);
            if (!empty($dayHandlingTimes)) {
                return $this->getMedianTime($dayHandlingTimes);
            }
        }

        // 3. Get overall average from last 30 days
        $thirtyDaysAgo = new \DateTime('-30 days');
        $now = new \DateTime();
        $allHandlingTimes = $this->getTicketHandlingTimes($thirtyDaysAgo, $now);

        if (!empty($allHandlingTimes)) {
            return $this->getAverageTime($allHandlingTimes);
        }

        // 4. Fallback default: 1 hour (60 minutes)
        return 60;
    }

    /**
     * Get handling times for tickets within a date range
     * Returns array of handling times in minutes
     */
    private function getTicketHandlingTimes(\DateTime $startDate, \DateTime $endDate)
    {
        $qb = $this->entityManager->createQueryBuilder();
        $qb->select('t.createdAt, t.updatedAt, s.code as statusCode')
            ->from(Ticket::class, 't')
            ->leftJoin('t.status', 's')
        ...
        foreach ($results as $ticket) {
            ...
            // Calculate handling time (use updatedAt as resolution time)
            if ($createdAt && $updatedAt) {
                $diff = $updatedAt->getTimestamp() - $createdAt->getTimestamp();
                $minutes = round($diff / 60); // Convert seconds to minutes

                // Only include reasonable handling times (between 1 minute and 7 days)
                if ($minutes >= 1 && $minutes <= 10080) { // 7 days = 10080 minutes
                    $handlingTimes[] = $minutes;
                }
            }
        }
    ...
    /**
     * Calculate median time from array of times
     */
    private function getMedianTime(array $times)
    {
        if (empty($times)) {
     ...
     */
    private function getAverageTime(array $times)
    {
        if (empty($times)) {
            return 60; // Default 1 hour
        }
    ...
    /**
     * Get dashboard statistics for ticket counts and active agents
     * Returns array with activeTickets, pendingTickets, closedTickets, activeAgents
     */
    public function getDashboardStats()
    {
        $qb = $this->entityManager->createQueryBuilder();
        
        // Get active tickets count (assuming status_id 1 = Open)
        $activeTickets = $qb->select('COUNT(t.id)')
            ->from(Ticket::class, 't')
            ->leftJoin('t.status', 's')
            ->where('s.id = 1') // Open tickets
            ->getQuery()
            ->getSingleScalarResult();

        // Get pending tickets count (assuming status_id 2 = Pending)
        $qb2 = $this->entityManager->createQueryBuilder();
        $pendingTickets = $qb2->select('COUNT(t.id)')
            ->from(Ticket::class, 't')
            ->leftJoin('t.status', 's')
            ->where('s.id = 2') // Pending tickets
            ->getQuery()
            ->getSingleScalarResult();

        // Get closed tickets count (assuming status_id IN (3,4,5) = Resolved/Closed)
        $qb3 = $this->entityManager->createQueryBuilder();
        $closedTickets = $qb3->select('COUNT(t.id)')
            ->from(Ticket::class, 't')
            ->leftJoin('t.status', 's')
            ->where('s.id IN (3,4,5)') // Closed/Resolved tickets
            ->getQuery()
            ->getSingleScalarResult();

        // Get active agents count (support role 1,2,3 = agents, exclude customers)
        $qb4 = $this->entityManager->createQueryBuilder();
        $activeAgents = $qb4->select('COUNT(DISTINCT ui.id)')
            ->from(UserInstance::class, 'ui')
            ->leftJoin('ui.supportRole', 'sr')
            ->leftJoin('ui.user', 'u')
            ->where('sr.id IN (1,2,3)') // Agent roles
            ->andWhere('ui.isActive = 1')
            ->andWhere('u.isEnabled = 1')
            ->getQuery()
            ->getSingleScalarResult();

        return [
            'activeTickets' => (int) $activeTickets,
    ...
    /**
     * Get ticket statistics data for charts based on time period
     * @param string $period '1D', '1M', '1Y', 'Max'
     * @return array with labels and data for chart
     */
    public function getTicketChartData($period = '1D')
    {
        $now = new \DateTime();
        $labels = [];
        $data = [];
        ...
        switch ($period) {
            case '1D':
                ...
                for ($i = 23; $i >= 0; $i--) {
                    ...
                    $startHour->setTime($hour->format('H'), 0, 0);
                    $endHour = clone $startHour;
                    $endHour->add(new \DateInterval('PT1H'));
                    
                    $labels[] = $hour->format('H:00');
                    $data[] = $this->getTicketCountBetweenDates($startHour, $endHour);
                }
                break;
            ...
            case '1M':
                ...
                for ($i = 29; $i >= 0; $i--) {
                    ...
                    $data[] = $this->getTicketCountBetweenDates($startDay, $endDay);
                }
                break;
            ...
            case '1Y':
                ...
                for ($i = 11; $i >= 0; $i--) {
                    ...
                    $startMonth = clone $month;
                    $startMonth->setDate($month->format('Y'), $month->format('n'), 1);
                    $startMonth->setTime(0, 0, 0);
                    $endMonth = clone $startMonth;
                    $endMonth->add(new \DateInterval('P1M'));
                    
                    $labels[] = $month->format('M Y');
                    $data[] = $this->getTicketCountBetweenDates($startMonth, $endMonth);
                }
                break;
            ...
            default:
                ...
                for ($i = 4; $i >= 0; $i--) {
                    $year = clone $now;
                    $year->sub(new \DateInterval("P{$i}Y"));
                    $startYear = new \DateTime($year->format('Y') . '-01-01 00:00:00');
                    $endYear = new \DateTime(($year->format('Y') + 1) . '-01-01 00:00:00');
                    
                    $labels[] = $year->format('Y');
                    $data[] = $this->getTicketCountBetweenDates($startYear, $endYear);
                }
                break;
        }
        ...
        return [
            'labels' => $labels,
            'data' => $data,
            'todayCount' => $this->getTodayTicketCount()
        ];
    }

    /**
     * Get count of tickets created between two dates
     */
    private function getTicketCountBetweenDates(\DateTime $startDate, \DateTime $endDate)
    {
        $qb = $this->entityManager->createQueryBuilder();
     ...
     */
    private function getTodayTicketCount()
    {
        $today = new \DateTime();
        ...
        return $this->getTicketCountBetweenDates($startOfDay, $endOfDay);
    }

    /**
     * Get agent activity statistics data for charts based on time period
     * @param string $period '1D', '1M', '1Y', 'Max'
     * @return array with labels and data for chart
     */
    public function getAgentActivityChartData($period = '1D')
    {
        $now = new \DateTime();
        $labels = [];
        $data = [];
        ...
        switch ($period) {
            case '1D':
                ...
                for ($i = 23; $i >= 0; $i--) {
                    ...
                    $labels[] = $hour->format('H:00');
                    $data[] = $this->getAgentActivityCountBetweenDates($startHour, $endHour);
                }
                break;
            ...
            case '1M':
                ...
                for ($i = 29; $i >= 0; $i--) {
                    ...
                    $labels[] = $day->format('M j');
                    $data[] = $this->getAgentActivityCountBetweenDates($startDay, $endDay);
                }
                break;
            ...
            case '1Y':
                ...
                for ($i = 11; $i >= 0; $i--) {
                    ...
                    $labels[] = $month->format('M Y');
                    $data[] = $this->getAgentActivityCountBetweenDates($startMonth, $endMonth);
                }
                break;
            ...
            default:
                ...
                for ($i = 4; $i >= 0; $i--) {
                    ...
                    $labels[] = $year->format('Y');
                    $data[] = $this->getAgentActivityCountBetweenDates($startYear, $endYear);
                }
                break;
        }
        ...
        return [
            'labels' => $labels,
            'data' => $data,
            'todayCount' => $this->getTodayAgentActivityCount()
        ];
    }

    /**
     * Get count of agent activities (ticket replies, updates) between two dates
     * This includes thread replies from agents and ticket status changes
     */
    private function getAgentActivityCountBetweenDates(\DateTime $startDate, \DateTime $endDate)
    {
        // Count agent thread replies (messages from agents)
        $qb1 = $this->entityManager->createQueryBuilder();
        $threadReplies = $qb1->select('COUNT(th.id)')
            ->from('Webkul\UVDesk\CoreFrameworkBundle\Entity\Thread', 'th')
            ->leftJoin('th.user', 'u')
            ->leftJoin('u.userInstance', 'ui')
            ->leftJoin('ui.supportRole', 'sr')
            ->where('th.createdAt >= :startDate')
            ->andWhere('th.createdAt < :endDate')
            ->andWhere('sr.id IN (1,2,3)') // Agent roles only
        ...
        // Count ticket status updates by agents (when agent updates ticket)
        $qb2 = $this->entityManager->createQueryBuilder();
        $ticketUpdates = $qb2->select('COUNT(t.id)')
            ->from(Ticket::class, 't')
            ->leftJoin('t.agent', 'a')
            ->leftJoin('a.userInstance', 'aui')
            ->leftJoin('aui.supportRole', 'asr')
            ->where('t.updatedAt >= :startDate')
            ->andWhere('t.updatedAt < :endDate')
            ->andWhere('t.updatedAt != t.createdAt') // Only count updates, not creation
            ->andWhere('asr.id IN (1,2,3)') // Agent roles only
            ->setParameter('startDate', $startDate)
            ->setParameter('endDate', $endDate)
            ->getQuery()
            ->getSingleScalarResult();

        return (int) ($threadReplies + $ticketUpdates);
    }

    /**
     * Get count of agent activities today
     */
    private function getTodayAgentActivityCount()
    {
        $today = new \DateTime();
        $startOfDay = clone $today;
        $startOfDay->setTime(0, 0, 0);
        ...
        return $this->getAgentActivityCountBetweenDates($startOfDay, $endOfDay);
    }
}

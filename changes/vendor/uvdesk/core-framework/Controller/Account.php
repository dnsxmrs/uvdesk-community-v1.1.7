...
class Account extends AbstractController
{
    ...
    public function loadDashboard(Request $request)
    {
        ...
        $dashboardStats = $this->userService->getDashboardStats();
        
        // Get initial chart data (1D by default)
        $ticketChartData = $this->userService->getTicketChartData('1D');
        $agentChartData = $this->userService->getAgentActivityChartData('1D');
    ...
    /**
     * AJAX endpoint for getting ticket chart data
     */
    public function getTicketChartDataXHR(Request $request)
    {
        $period = $request->get('period', '1D');
        $chartData = $this->userService->getTicketChartData($period);
        
        return new Response(json_encode($chartData), 200, [
            'Content-Type' => 'application/json'
        ]);
    }

    /**
     * AJAX endpoint for getting agent activity chart data
     */
    public function getAgentActivityChartDataXHR(Request $request)
    {
        $period = $request->get('period', '1D');
        $chartData = $this->userService->getAgentActivityChartData($period);
